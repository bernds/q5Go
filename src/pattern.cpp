...
go_pattern::go_pattern (unsigned sz_x, unsigned sz_y)
	: m_lines (sz_y), m_sz_x (sz_x), m_sz_y (sz_y)
{
}
...
std::vector<cand_match> go_pattern::find_cands (const bit_array &other_w, const bit_array &other_b, const bit_array &other_caps,
	...
	for (unsigned yoff = min_offy; yoff < max_offy; yoff++) {
		...
		for (unsigned xoff = min_offx; xoff < max_offx; xoff++, valid_a >>= 1, valid_b >>= 1) {
			if (valid_a & 1) {
				...
				cands.emplace_back (*this, std::move (caps), xoff, yoff);
			}
			if (valid_b & 1) {
				std::vector<uint32_t> caps (m_sz_y);
				for (unsigned y = 0; y < m_sz_y; y++)
					caps[y] = other_caps.extract ((yoff + y) * other_sz_x + xoff, m_sz_x);
				cands.emplace_back (*this, std::move (caps), xoff, yoff, cand_match::swapped);
			}
		}
	}
	return cands;
...
bool match_movelist (const std::vector<char> &moves, std::vector<cand_match> &cands,
		     std::vector<gamedb_model::cont_bw> &conts, int cont_maxx, coord_transform reverse)
{
	...
	for (auto &it: cands) {
		if (it.was_continued ()) {
			...
			std::tie (x, y) = reverse (x, y, it.sz_x (), it.sz_y ());
			if (col == 0)
...
bit_array match_games (const std::vector<unsigned> &cand_games, const go_pattern &pat,
		       std::vector<gamedb_model::cont_bw> &conts, int cont_maxx, coord_transform reverse,
		       std::atomic<long> *count)
{
	...
	int local_count = 0;
	for (int i: cand_games) {
		auto &entry = db_data->m_all_entries[i];
		auto cand_matches = pat.find_cands (entry.finalpos_w, entry.finalpos_b, entry.finalpos_c,
		...
		if (entry.movelist.size () > 0) {
			if (match_movelist (entry.movelist, cand_matches, conts, cont_maxx, reverse))
				result.set_bit (i);
			continue;
		}
		...
		if (f.exists ()) {
			...
			if (ds.readRawData (i_c, sizeof i_c) == sizeof i_c) {
				...
				if (ds.readRawData (&movelist[0], len) == len) {
					if (match_movelist (movelist, cand_matches, conts, cont_maxx, reverse))
						result.set_bit (i);
				}
			}
			f.close ();
...
class PartialSearch : public QRunnable
{
	...
	bool m_skip;
public:
	PartialSearch (std::vector<unsigned> *e, const go_pattern &p, bit_array *r,
		       std::vector<gamedb_model::cont_bw> *c, QMutex *m, QSemaphore *s,
		       std::atomic<long> *count, std::atomic<long> *max)
		: m_pat (p.transform<T> ()), m_entries (e), m_mutex (m), m_sem (s), m_result (r), m_conts (c),
		  m_pcur (count)
	{
		m_cont_sz_x = p.sz_x ();
		setAutoDelete (true);

		m_skip = !T::is_identity && p == m_pat;
		if (m_skip)
			*max -= e->size ();
	}
	void run () override
	{
		if (!m_skip) {
			std::vector<gamedb_model::cont_bw> continuations (m_pat.sz_y () * m_pat.sz_x ());
			auto games = match_games (*m_entries, m_pat, continuations, m_cont_sz_x, T::inverse::transform,
						  m_pcur);
			{
...
gamedb_model::find_pattern (const go_pattern &p, std::atomic<long> *cur, std::atomic<long> *max)
{
	bit_array result (db_data->m_all_entries.size ());
	std::vector<cont_bw> continuations (p.sz_y () * p.sz_x ());

	QMutex result_mutex;
	QSemaphore completion_sem (0);
	QThreadPool pool;
	max->store (m_entries.size () * 8);
	pool.start (new PartialSearch<coord_transform_none> (&m_entries, p, &result, &continuations, &result_mutex, &completion_sem, cur, max));
	pool.start (new PartialSearch<coord_transform_r90> (&m_entries, p, &result, &continuations, &result_mutex, &completion_sem, cur, max));
	pool.start (new PartialSearch<coord_transform_r180> (&m_entries, p, &result, &continuations, &result_mutex, &completion_sem, cur, max));
	pool.start (new PartialSearch<coord_transform_r270> (&m_entries, p, &result, &continuations, &result_mutex, &completion_sem, cur, max));
	pool.start (new PartialSearch<coord_transform_flip1> (&m_entries, p, &result, &continuations, &result_mutex, &completion_sem, cur, max));
	pool.start (new PartialSearch<coord_transform_flip2> (&m_entries, p, &result, &continuations, &result_mutex, &completion_sem, cur, max));
	pool.start (new PartialSearch<coord_transform_flip3> (&m_entries, p, &result, &continuations, &result_mutex, &completion_sem, cur, max));
	pool.start (new PartialSearch<coord_transform_flip4> (&m_entries, p, &result, &continuations, &result_mutex, &completion_sem, cur, max));

	completion_sem.acquire (8);
