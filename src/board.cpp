...
game_state *Board::analysis_at (int x, int y, int &num, double &primary_wr, double &primary_score)
{
	...
	if (next != nullptr && next->get_move_x () == x && next->get_move_y () == y) return next;
	return nullptr;
}
						...
						int x, int y, bool child_mark, int v, int max_number)
{
	...
	if (pv_idx > 0) {
		...
		/* Use green for a difference of 0, red for any loss bigger than 12%.
		   The HSV angle between red and green is 120, so just multiply winrate by 1000 or
		   score difference by 40. */
		double angle_off = analysis_vartype < 3 ? 1000 * wr_diff : 40 * score_diff;
		int angle = std::min (120.0, std::max (0.0, 120 + angle_off));
	...
	if (analysis_vartype == 0) {  // letters
		...
		svg.text_at (cx, cy, svg_factor, 4,
			     QString::number (shown_val * 100, 'f', 1),
			     "black", fi);
	} else if (analysis_vartype == 3) {  // score (absolute)
